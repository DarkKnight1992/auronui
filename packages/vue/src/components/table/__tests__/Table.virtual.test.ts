...
function mountTable(
  ...
  const Wrapper = defineComponent({
    components: { Table },
    setup() {
      return { columns, data, tableProps: props }
    },
    template: '<Table :columns="columns" :data="data" v-bind="tableProps" />',
  })
  return mount(Wrapper, mountOptions.attachTo ? { attachTo: mountOptions.attachTo } : undefined)
}
...
describe('Table — virtualization', () => {
  ...
  it('virtualRows=true renders <tbody> (virtualized mode)', () => {
    ...
    const wrapper = mountTable(data, { virtualRows: true }, { attachTo: document.body })
  ...
  it('virtualRows=50 threshold: data.length=30 renders non-virtual <tbody>', () => {
    ...
    const wrapper = mountTable(data, { virtualRows: 50 })
    expect(wrapper.find('tbody').exists()).toBe(true)
  ...
  it('virtualRows=50 threshold: data.length=100 renders virtualized <tbody>', () => {
    const data = makeData(100)
    const wrapper = mountTable(data, { virtualRows: 50 }, { attachTo: document.body })
    expect(wrapper.find('tbody').exists()).toBe(true)
    expect(wrapper.findAll('tbody tr[role="row"]').length).toBeLessThan(100)
    wrapper.unmount()
  })
  ...
  it('virtualized mode preserves selection state for initially-visible rows', async () => {
    ...
    const wrapper = mountTable(
      data,
      { virtualRows: true, selection: 'multiple', rowSelection: { '0': true } },
  ...
  it('switching virtualRows from false to true still renders <tbody>', async () => {
    const data = makeData(100)
    const virtualRows = ref<boolean>(false)
    const Wrapper = defineComponent({
      components: { Table },
      setup() {
        return { columns, data, virtualRows }
      },
      template: '<Table :columns="columns" :data="data" :virtual-rows="virtualRows" />',
    })
    const wrapper = mount(Wrapper, { attachTo: document.body })
    expect(wrapper.findAll('tbody tr[role="row"]').length).toBe(100)
    virtualRows.value = true
    await nextTick()
