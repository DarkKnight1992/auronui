/**
 * Ported 1:1 from packages/vue/src/components/form/validation.ts — framework
 * agnostic, no Vue/React dependency in the original, so the port is a literal
 * TS-syntax translation (no logic changes, same default messages) to keep
 * validation-error-message parity between the two packages.
 */
...
function resolvePatternRule(
  rule: RegExp | { value: RegExp; message: string },
): { value: RegExp; message: string | undefined } {
  if (rule instanceof RegExp) return { value: rule, message: undefined };
  return rule;
}

function resolveStringRule(
  rule: string | { value: string; message: string },
...
/**
 * Adapts Auron's `FieldRules`/`CustomValidator` pair into a react-hook-form
 * `validate` function: RHF expects `true` for "valid" and a string for
 * "invalid, here's the message" — the inverse of runValidation's
 * `undefined` = valid convention.
 */
export function buildRHFValidate(
  rules: FieldRules | undefined,
  validate: CustomValidator | undefined,
  getValues: () => Record<string, unknown>,
): ((value: unknown) => Promise<true | string>) | undefined {
  if (!rules && !validate) return undefined;
  return async (value: unknown) => {
    const message = await runValidation(value, rules, validate, { values: getValues() });
    return message ?? true;
  };
}
