...
### 2026-09-05
- **Rule established: no component may require a `class` to render as intended.** Props are fine; a consumer-supplied class is only ever additive. Audit of both packages (stories, docs, and every `*.styles.ts` slot) found four violations, all fixed:
  - **`ModalClose` / `DrawerClose` corner X needed a hand-pasted class.** `a28028f` (a color commit) had dropped `styles.closeTrigger()` from `ModalClose` and pasted `class="modal__close-trigger"` onto 21 story sites instead; Drawer stories hardcoded `absolute top-3 right-3` on 12. Root cause: at v1.0.0 `ModalClose` was used *only* in footers while `.modal__close-trigger` was `absolute top-4 right-4`, so every footer Close button flew to the corner — the class was never wrong, it was just unscoped. **Fix is CSS-only scoping, no new component/prop/class:** `.modal__close-trigger { absolute top-3 right-3 }` by default, reset to `static` under `.modal__footer` (same for drawer). Chosen over `.modal__dialog > .modal__close-trigger` deliberately: the child combinator breaks an X placed in the header or body, whereas the footer-reset lets it land in the corner from the dialog root, header, *or* body — verified in a real browser: all three measure 12/12 from the dialog, footer measures `static` in flow. Works because none of header/body/footer establish a containing block (no `position`/`transform`/`filter`), so an absolute X anchors to `.modal__dialog` (`relative`) / `.drawer__dialog` (`fixed`; `--inline` absolute, `--dock` relative). Bonus: an X in a `scroll-inside` body escapes the scroller instead of scrolling away. **INVARIANT (also commented in both CSS files): any container that lays close triggers out inline must reset them.** `ModalClose.vue` re-applies `styles.closeTrigger()`; React's `ModalClose`/`DrawerClose` merge it into the cloned child's `className`. Modal X moved from `top-4 right-4` to `top-3 right-3` per explicit request (unifies with Drawer).
  - **`ToastAction`** applied bare `:class="props.class"` — `toast__action` was never on the DOM (Vue + React `ToastProvider`).
  - **React `Avatar` never rendered the `avatar__fallback` wrapper** — initials/icon went straight into the root, losing `flex size-full items-center justify-center bg-default` and every per-color `solid`/`soft` fill. Now wraps all three fallback branches like Vue does. **React `SidebarItem`** badge Chip was missing `sidebar__item-badge`.
  - `Link` stories used `class="link--underline-*"` where the `underline` prop already exists — switched.
- **Lint guard: `packages/styles/src/__tests__/slot-coverage.test.ts`.** Every slot in a `*.styles.ts` must be applied (`.slot(` call or literal BEM class, `__tests__` excluded) by the owning component dir in *both* packages. Ships with a frozen BASELINE of pre-existing unapplied slots (52 Vue / 75 React) that may only shrink — a second assertion fails on stale entries so it self-cleans. Verified it catches the `a28028f` regression by reverting `ModalClose` and watching it fail. The rule of thumb for a failure: fix the component, never grow the baseline. Baseline entries that are *structural*, not styling debt, worth knowing before "fixing": React `Calendar` delegates the month view to `MonthPicker` (its own `month-picker__*` classes, so `calendar.monthGrid*` never applies there); RAC's `CalendarCell` is one element, so `calendar.cellButton` (`absolute inset-0`, meant for an inner button) cannot be applied to it without breaking the grid; `modal.portal` is Vue's fixed layer wrapper, RAC layers on its own. Genuine React feature gaps still open: Tabs overflow menu (6 slots), DateTimePicker timezone panel (4), Calendar/RangeCalendar year grids.
- Tooling notes from this session: `pnpm` is not on PATH in the sandboxed shell — use `packages/<pkg>/node_modules/.bin/{vitest,vite,tsc}` directly. The installed Playwright Chromium (`~/.cache/ms-playwright/chromium-1234`) does not match playwright 1.59.1's expected revision 1217, so `chromium.launch()` fails; pass `executablePath: ~/.cache/ms-playwright/chromium-1234/chrome-linux64/chrome` and import playwright by absolute path from `node_modules/.pnpm/playwright@1.59.1/node_modules/playwright/index.js` (ESM ignores `NODE_PATH`). Full-suite runs have two timing flakes that pass in isolation: Vue `form/__tests__/exports.test.ts`, React `NavigationMenu` "clicking an open trigger again".

### 2026-09-02
- **FormControl** (new) — `packages/vue/src/components/form/FormControl.vue`, per the approved spec in `docs/superpowers/specs/2026-09-01-form-control-design.md` (gitignored, local only). `<FormControl name="auth_factor.force_mfa" :as="Checkbox">` renders `FormField` and binds its `fieldProps` onto the control named by `as`, so **no per-input wrapper is needed**. This is a correctness fix, not just ergonomics — see below.
  - **The bug it exists to prevent.** A consumer wrapper declaring `defaultValue?: boolean` compiles to `{ type: Boolean, required: false }`, and Vue casts an **absent** Boolean prop to `false`, not `undefined`. The wrapper therefore forwards `:default-value="false"` on every render where its own author passed nothing; `FormField`'s (correct) precedence makes that beat the form-level default, and the field *registers* as `false`, so a save writes `false` over the server's value. Only Boolean props are affected — `number`/`string` get no absent-cast, which is why numeric and text fields in the same form seed fine and only checkboxes break. **This was the real cause of a "form-level defaultValues don't apply" report that survived the 1.10.4 reactivity fix** — reproduced and confirmed failing *synchronously*, with defaults present at mount, which is what proved it was never the async path. `FormControl` types its own `defaultValue` as `unknown` (compiles to `{ type: null }`, never cast), and the consumer declares no props at all.
  - **Binds only what the control declares**, read at runtime off `(as as ComponentOptions).props` (normalising the array form, tolerating a component with no `props`). `modelValue`/`onUpdate:modelValue`/`onBlur` always; `name`/`isInvalid`/`isDisabled`/`errorMessage` only when declared. Load-bearing, not padding: `Checkbox`/`Switch` declare no `errorMessage`, so binding it unconditionally falls through to `$attrs` and renders a stray `errormessage` attribute. `onBlur` is unconditional because it is a listener — every control sets `inheritAttrs: false` and re-binds attrs onto its inner element, so it reaches a real blur (needed for `validation-mode="on-blur"`).
  - **`$attrs` booleans must be coerced by hand.** `isDisabled` is OR-ed between `fieldProps.isDisabled` and a consumer-passed `is-disabled` so neither a form-level nor a per-control value can defeat the other — but a bare `is-disabled` in a template arrives in `$attrs` as `""`, not `true`, because it is not a declared prop here and Vue never normalised it. Caught by a failing test; `attrFlag()` applies Vue's own rule (`true`, `''`, or the key name) across both casings.
  - **Dev-only warning in `FormField`** (`warnDefaultValueShadow` in `utils/warnDeprecated.ts`) for anyone who still hand-rolls: fires only when the field-level default is `false` **and** a form-level default exists for that name **and** is not `false`. Deliberately narrow — the Boolean cast can only ever manufacture `false`, so wider conditions would nag on legitimate per-field overrides. Resolved through `ctx.getDefaultValue(name)`, not an own-property check, so nested/dotted names are seen (a deviation from the spec, which predates the 1.10.4 path work).
  - **Pre-existing accessibility defect surfaced, NOT introduced, and NOT fixed here.** reka-ui renders a visually-hidden native `<input>` (`VisuallyHiddenInput`) whenever a `name` is bound *and* the control is inside a `<form>`. That input has no label — and inside `CheckboxRoot` it is nested in the `role="checkbox"` button — so axe reports `label` (Checkbox, NumberField, and any other reka control with the shim) plus `nested-interactive` (Checkbox). Measured identical for the pre-existing hand-written `FormField` + `v-bind="fieldProps"` pattern, which binds `name` too, so it is not FormControl's doing; `CheckboxRoot` passes a fixed prop set to that input with **no `$attrs` pass-through**, so it cannot be corrected from our wrapper components. `FormControl.axe.test.ts` therefore asserts a clean bill for text `Input` and a **parity** assertion for Checkbox/NumberField (same violation ids via `FormControl` as via `FormField`, plus the known id set pinned) — it will fail the day the underlying issue is fixed, which is when we want to know. Worth a separate decision: patch upstream, drop `name`, or accept.
  - Tests: `__tests__/FormControl.test.ts` (22 — seeding incl. the async path, the full precedence table, `undefined` vs `null`, `v-model` outranking, declared-prop filtering incl. asserting no `errormessage` in the DOM, `isDisabled` OR-ing both ways, attr/slot pass-through, on-blur validation, a control declaring no props, and the five warning cases) and `__tests__/FormControl.axe.test.ts` (6). Storybook story "FormControl (seeded from :default-values)" and docs page `apps/docs/components/form-control.md`.

### 2026-09-01
- **Form** — the value shape is now single-valued: a field's registered name is a path, and flat, nested and deeply nested names all behave the same everywhere. Previously "nested support" was re-derived at each boundary and each one chose differently, because **`Form` has no central store** — each field owns its own `ref` (`FieldRegistration.valueRef`) and `form.state.ts` keeps only a registration `Map` keyed by the literal name, with `values` a *derived* computed that re-walks the registry. (react-hook-form has one canonical `_formValues` object and one `get`/`set` pair, which is why nesting is free there and had to be built here.)
  - **New `packages/vue/src/utils/path.ts`** — `getPath`/`setPath`/`flattenPaths`/`toPathSegments`, the single path layer every form read and write goes through (`getPath`/`setPath` are exported publicly from the package root). A literal own key still wins over walking, so flat maps keyed by a dotted name keep working. `setPath` creates an array when the next segment is numeric (`contacts.0.email`), and both guard `__proto__`/`constructor`/`prototype`. `flattenPaths` treats a class instance as a leaf, not a subtree — a `Date` value must not be walked into.
  - **`getAllValues()` is gone.** It existed for exactly one reason: cross-field `matches`/`deps` lookups needed to resolve a sibling by its exact registered name, and **field-array row ids are not paths in the public shape** (`contacts.contacts-row-0.email` becomes `contacts[0].email` in `getValues()`). Rather than pick flat or nested, sibling lookup now goes through the **registry** — `ctx.getFieldValue(name)` hits `fields.get(name)` first and falls back to a path read — which resolves row ids and ordinary dotted paths alike and needs no flat snapshot. `ValidationContext` gained an optional `getFieldValue`; `matches` prefers it and falls back to `getPath(values, name)`.
  - **The shape a rule sees is now the same on every trigger.** It genuinely was not before: on change/blur `useField`/`FormField` passed `ctx.getValues()` (nested) while submit/`trigger()` passed `getAllValues()` (flat) — measured as `{"a":{"b":"y"}}` vs `{"a.b":"y"}` for the same field. A cross-field rule written against one silently broke under the other. `buildValidationContext()` now assembles the nested tree **once per validation pass**, not per field.
  - **`buildNestedValues` was refactored onto `setPath`** via a new `resolveValuePath(name)` that translates a registered name into its public path (row id → current index, `null` for a stale row). The row-id indirection stays deliberate — it is what keeps a row's field names stable across reorders so reordering never remounts them (see the 2026-07-13 entry); react-hook-form instead names rows by index and absorbs the renaming centrally in its own `useFieldArray`.
  - **API additions**: `getValues(name)` reads a leaf or a whole subtree by path; `setValue(name, value)` fans a subtree write out to the registered fields it covers (`setValue('auth_factor', { force_mfa: true })` reaches `auth_factor.force_mfa`). Field-array structural writes still go through `append`/`remove`/etc., not `setValue`, since rows are addressed by id. **`errors` stays keyed by literal registered name** — it is a lookup by field identity, not a value shape, and every field reads its own error by its own name.
  - Tests: `packages/vue/src/utils/__tests__/path.test.ts` (21) and `packages/vue/src/components/form/__tests__/nested-values.test.ts` (6, including the field-array `matches`-across-row-ids case the flat shape existed to serve, and an assertion that change/blur and submit hand over byte-identical `values`).
- **Form** — fixed two independent defects in `defaultValues` resolution (`packages/vue/src/components/form/`), both of which had to be present for a nested, API-fetched default object to populate anything.
  - **Dotted field names were never resolved against nested `defaultValues`.** `useField`/`FormField`/`useFieldArray` all read `ctx.defaultValues[name]` — a flat property access — so a field named `auth_factor.force_mfa` looked up the literal key `"auth_factor.force_mfa"` and got `undefined` against `{ auth_factor: { force_mfa: true } }`. The form was **asymmetric**: `buildNestedValues()` in `form.state.ts` already walked dotted names to *assemble* a nested output shape, but nothing walked them on the way *in*. Fixed with `resolveDefault()` in `form.state.ts`, exposed as a new `ctx.getDefaultValue(name)` — a literal key still wins (so flat maps keyed by a dotted name keep working), otherwise the name is walked as a path.
  - **`defaultValues` was a one-time snapshot, so async/fetched defaults never landed.** `createFormState` destructured `const { defaultValues = {} } = options` at creation and `Form.vue` passed `props.defaultValues` by value, so the context permanently held whatever object existed at setup — usually `undefined` while the API call was in flight. Fixed by widening the option to `MaybeRefOrGetter` (`Form.vue` now passes `toRef(props, 'defaultValues')`, and `FormOptions.defaultValues` is widened the same way for `useForm()` callers), reading through a `computed`. `ctx.defaultValues` is **kept on the context as a `Proxy`** whose `get`/`has`/`ownKeys` read the live source — that preserves the existing `Record<string, unknown>` type and every `ctx.defaultValues[x]` call site while making those reads reactive. Note the tracking granularity: replacing the whole object always re-triggers; mutating a nested property in place only does when the source object is itself deeply reactive.
  - **Adoption is gated on identity with the *previous* default, not on `dirty`.** Fields seed from `undefined` at mount, so a watcher has to write the late-arriving value in. The guard is `if (modelValue !== undefined && modelValue !== previousDefault) return` — the field adopts only while it still holds exactly what the last default gave it. This is deliberately stricter than gating on `dirty`: `dirty` is derived from a `watch(modelValue)` comparison, so any control that normalizes its own value on mount (a checkbox emitting `false` for `undefined`, say) would flip it and silently block adoption forever, and in `FormField` a parent-supplied `v-model` value would be clobbered. `useFieldArray` re-seeds under the analogous guard — only while `arraysEqual(order, initialOrderSnapshot)`, i.e. no rows added/removed/reordered.
  - **`@auronui/react` is unaffected and was not changed** — its Form delegates to `react-hook-form`, which resolves nested paths natively; its `defaultValues` being mount-only is that library's documented behavior (callers use `values` instead), not a defect here.
  - Regression tests: `packages/vue/src/components/form/__tests__/default-values.test.ts` (6 cases — nested resolution, flat-literal-key fallback, late arrival, user-edit protection, `getValues()` round-trip, `useField()` parity).
